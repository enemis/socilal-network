...
type Queue struct {
	...
	AutoAck   bool
	Durable   bool
	DelUnused bool // Delete when unused
	Exclusive bool
	NoWait    bool       // No-wait
	Arg       amqp.Table // Arguments
}
...
func newClient(queue Queue, logger logger.LoggerInterface, addr string) *Client {
	client := Client{
		m:      &sync.Mutex{},
		logger: logger,
		Queue:  queue,
...
func NewFactory(cfg *config.Config, logger logger.LoggerInterface) *ClientFactory {
	return &ClientFactory{config: cfg, logger: logger}
}

func (s *ClientFactory) NewClient(queue Queue) *Client {
	return newClient(queue, s.logger, s.config.RabbitMqAddr)
}
...
func (client *Client) handleReconnect(addr string) {
	for {
		...
		client.logger.Info("attempting to connect", nil)

		conn, err := client.connect(addr)
		if err != nil {
			client.logger.Info("failed to connect. Retrying...", nil)
...
func (client *Client) connect(addr string) (*amqp.Connection, error) {
	...
	client.logger.Info("client amqp connected", nil)
	return conn, nil
}
...
func (client *Client) handleReInit(conn *amqp.Connection) bool {
	for {
		...
		if err != nil {
			client.logger.Info("failed to initialize channel, retrying...", nil)

			select {
			case <-client.done:
				return true
			case <-client.notifyConnClose:
				client.logger.Info("connection closed, reconnecting...", nil)
				return false
		...
		case <-client.notifyChanClose:
			client.logger.Info("channel closed, re-running init...", nil)
		}
	}
}
...
func (client *Client) init(conn *amqp.Connection) error {
	...
	client.isReady = true
	client.m.Unlock()
	client.logger.Info("client init done", nil)
...
func (client *Client) Push(data []byte) error {
	...
	for {
		...
		if err != nil {
			client.logger.Info("push failed. Retrying...", nil)
			select {
			case <-client.done:
				return errShutdown
		...
		if confirm.Ack {
			client.logger.Info(fmt.Sprint("push confirmed [%d]", confirm.DeliveryTag), nil)
			return nil
		}
	}
}
